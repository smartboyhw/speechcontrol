...
void Training::startTraining ( Session* p_session ) {
    if ( !p_session->isCompleted() ) {
        ...
        MESSAGEBOX::information ( Core::mainWindow() ,tr ( "Session Completed" ), tr ( "The session %s has been completed already." ).arg ( p_session->content()->title() ) );
}
...
void Training::startCollecting() {
    ...
    if ( m_curSntct ) {
        ...
        reject();
        hide();
        MESSAGEBOX::information ( this,tr ( "Session Completed" ),tr ( "<i>No text is available for this session</i>." ) );
    }
}
...
void SpeechControl::Windows::Training::on_pushButtonNext_clicked() {
    // Finalize this part.
    Sentence* l_nextSentence = m_session->firstIncompleteSentence();
